...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    if [[ "$(command -v apt)" != "" ]]; then
        ...
        $sudo_cmd zypper install -y unrar zip unzip sharutils uudeview arj cabextract file-roller dtc python3-pip brotli axel aria2 renameutils cpio lz4 python3-devel xz-devel p7zip
...
"$PIP" install aospdtgen backports.lzma extract-dtb protobuf pycryptodome docopt zstandard build setuptools pip
